...
const websocketUrl =
  "wss://[websocket-api-id].execute-api.ap-southeast-2.amazonaws.com/prod";
let ws;
let currentJobId;
...
async function handleUpload() {
  ...
  try {
    ...
    const response = await fetch(
      `/upload-url?filename=${encodeURIComponent(filename)}&contentType=${encodeURIComponent(file.type)}`,
    );
    ...
    if (uploadResponse.ok) {
      showStatus(
        `Processing started (Job: ${jobId})... You will be notified when complete.`,
      );
    } else {
